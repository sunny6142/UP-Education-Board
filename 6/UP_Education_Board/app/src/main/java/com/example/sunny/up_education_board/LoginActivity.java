...
public class LoginActivity extends AppCompatActivity  {
 ...
 //   public SimpleDateFormat dateForm  at = new SimpleDateFormat("dd MM yyyy", /*Locale.getDefault()*/Locale.ENGLISH);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final DatabaseReference timeRef = FirebaseDatabase.getInstance().getReference("timetest").child(key).child("time");
        ...
        timeRef.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                if (dataSnapshot.exists()) {
                    ...
                    Moniter.DD = Manage.tdate;
                    Moniter.DD = Moniter.DD.replace(".", "");
                    String part[] = Moniter.DD.split(" ");
                    Moniter.DD = part[0] + " " + part[1] + " " + part[2] + " " + part[3] + " " + part[4].toUpperCase();

                    Log.v("ABCD login 0 ", "onDataChange: " + Moniter.DD);
                    //    Moniter.DD = Manage.tdate.replace(".","").toUpperCase();////////////////
                    //    Moniter.DD = Moniter.DD.replace("-"," ").toUpperCase();////////////
                    //   Log.v("ABCD login 1", "onDataChange: " + Manage.tdate);
                    Manage.tdate = Manage.tdate.replace("-", " ");
                    Manage.tdate = Manage.tdate.replace(".", "");
                    String part2[] = Manage.tdate.split(" ");
                    Manage.tdate = part2[0] + " " + part2[1] + " " + part2[2] + " " + part2[3] + " " + part2[4].toUpperCase();
                    Log.v("ABCD login 10 ", "onDataChange: " + Manage.tdate);
                    timeRef.removeEventListener(this);
                    //   Manage.tdate = Manage.tdate.replace(".","").toUpperCase();///////////
                    //  Manage.tdate = Manage.tdate.replace("-"," ").toUpperCase();//////////////
    ...
    public void GetValue(final String email, final String password , final ProgressDialog pd)
    {
        ...
        this.myRef.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                if(dataSnapshot.child(email).hasChild(password))
                {
                    Map<String ,Object> key = (Map<String, Object>) dataSnapshot.child(email).getValue();
                    disms(pd);
                    sname = email;
                    schoolcode = (String) key.get(password);
                    StoreData.schoolcode = schoolcode;

                 //   Log.v("abcd",schoolcode);
                    db.addUser("1",email,schoolcode);
